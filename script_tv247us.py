...
def descubrir_proxy_domain_100_dinamico(res_html, js_texts, active_host, base_site):
    ...
    # 1. Consultar APIs de resolución dinámicas encontradas en el código fuente
    ...
    # 3. Subdominios del sitio web presentes en el código JS/HTML
...
def obtener_canales_y_proxy():
    ...
    for site in BASE_SITES:
        print(f"Probando sitio fuente TV247US: {site}...")
        try:
    ...
    if not res:
        raise ConnectionError("No se pudo conectar a ninguno de los dominios base especificados para TV247US.")
    ...
    combined_code = res.text + '\n' + '\n'.join(js_texts)

    # Buscar el bundle de canales específicamente
    channels_js_text = ''
    ...
    if not channels_js_text:
        match_ch = re.search(r'assets/channels-[a-zA-Z0-9_-]+\.js', combined_code)
        if match_ch:
            try:
                r_ch = requests.get(f"{base_site_usado}/{match_ch.group(0)}", headers=HEADERS, timeout=5)
                if r_ch.status_code == 200:
                    channels_js_text = r_ch.text
            except Exception:
                pass

    if not channels_js_text:
        channels_js_text = combined_code
...
def generar_lista_m3u(canales, proxy_domain, filename=OUTPUT_M3U):
    ...
    print(f"¡Éxito! Lista M3U para TV247US generada correctamente con {len(canales)} canales usando el dominio '{proxy_domain}'.")
